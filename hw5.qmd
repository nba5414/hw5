...
df <- read_csv(path) %>% as_tibble() %>%         mutate_at('ocean_proximity', as.factor) %>%
  drop_na()
...
```{R}
df %>% select(!ocean_proximity) %>% cor() %>% corrplot() 
```
